                                      ...
                                      const char* cmd, const char* arg)
{
  ...
  if (sb.bind_until_success (portno)) {
    sock_error ("ftpdata: ", "bind_until_success failed");
    exit (1);
  }
  ...
  if (o) {
    ...
    c.close (); // this close is needed. Otherwise, c will not send
                // a response.
  }
...
// right now no server ftp class cannot be used as a server
{}
...
ftp::replycodea ftp::ftpbuf::useraddr (sockinetaddr sa)
{
  if (sa.sin_addr.s_addr == 0) {
    ...
    if (::gethostname (hostname, 63) == -1) {
      perror ("ftpbuf::useraddr");
      return ftp::rca_error;
    }

    hostent* hp = gethostbyname (hostname);
    if (hp == 0) {
      sock_error ("ftpbuf: ", "gethostbyname");
      return ftp::rca_error;
    }
