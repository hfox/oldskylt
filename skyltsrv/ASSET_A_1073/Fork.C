...
Fork::ForkProcess::ForkProcess (int kill, int give_reason)
     : kill_child (kill), reason (give_reason), next (0)
{
  if (list == 0) {
    struct sigaction sa;
    sa.sa_handler = funcptr (&Fork::ForkProcess::reaper_nohang);
  ...
  if (pid > 0) {
    ...
    if (kill_child) {
      struct sigaction sa;
      sa.sa_handler = funcptr (&Fork::ForkProcess::commit_suicide);
      sigemptyset (&sa.sa_mask);
      sa.sa_flags = SA_RESTART;
